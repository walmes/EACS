...
#'     3 \eqn{\times} 2 m. Cada unidade experimental foi constituida de
#'     136 plantas, contendo 50 plantas úteis por parcela. O
#'     delineamento experimental é de blocos casualizados (DBC) com 6
#'     repetições. O experimento possui dois (02) fatores de estudo, que
#'     são manejo do solo e profundidade do solo. Para o fator manejo
#'     são quatro (04) níveis de estudo e para o fator profundidade do
#'     solo foram nove (09) níveis. O levantamento destes dados foi
#'     feito em abril/maio de 2016, na lavoura com 2 anos de idade.
#'
...
#'     camada do solo com 9 camadas de espessura 5 cm. As amostras de
#'     solo foram extraídas de forma continua no mesmo ponto geográfico
#'     ao longo do perfil do solo. Os níveis são: 0-0,05; 0,05-0,10;
...
#'     m.}
#'
#' \item{\code{umid0}}{Umidade volumétrica (cm cm\eqn{^{-3}}), com
#'     amostra indeformada, do solo na tensão de 0 kPa (solo hidratado
#'     por capilaridade até a saturação).}
#'
#' \item{\code{umid6}}{Umidade volumétrica (cm cm\eqn{^{-3}}), com
#'     amostra indeformada, para solo submetido à tensão de 6 kPa
#'     (considerado umidade na capacidade de campo), .}
#'
#' \item{\code{rp}}{Resistência do solo à penetração (MPa).}
#'
#' \item{\code{dens}}{Densidade do solo (Mg m\eqn{^{-3}}).}
#'
#' }
#'
#'     A amostra indeformada proveniente do campo foi primeiramente
#'     submetida à hidratação para determinação das umidades (em 0 e 6
#'     kPa) e depois foi feita determinação da resistência à penetração
#'     que deforma a amostra. Por fim, foi feita a determinação da
#'     densidade do solo que não querer amostra indeformada.
