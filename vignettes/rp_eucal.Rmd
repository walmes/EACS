...
txt <-
"Diagrama de dispersão da umidade (g g$^{-1}$) em 0 e 6 kPa em
...
xyplot(umid0 + umid6 ~ cam | manejo,
       data = rp,
       xlab = "Profundidade (cm)",
       ylab = expression("Umidade do solo" ~ (g ~ g^{-1})),
       type = c("p", "smooth"),
       auto.key = TRUE)
```
...
```{r, echo = FALSE, fig.cap = cap}
txt <-
"Correlação entre camadas em função da distância (cm) entre elas no
 perfil do solo."
...
curve(exp(-x/r),
      ...
      xlab = "Distância entre camadas do solo (cm)")
abline(v = 3 * r, h = exp(-3), lty = 2)
```

A estimativa de alcance[^1] foi de `r 3*r` cm
...
```{r, echo = FALSE, fig.cap = cap}
txt <-
"Valores observados de umidade do solo (g g$^{-1}$) em 0 kPa em
...
xyplot(umid0 ~ cam | manejo,
       data = rp[-63, ],
       ylab = expression("Umidade em 0 kPa" ~ (g ~ g^{-1})),
       xlab = "Profundidade (cm)") +
...
A estimativa de alcance foi de `r 3*r` cm
...
```{r, echo = FALSE, fig.cap = cap}
txt <-
"Valores observados de umidade do solo (g g$^{-1}$) em 6 kPa em
 função da profundidade do solo (cm) para cada manejo com a curva
 ajustada acompanhada das bandas de confiança (95%)."
...
xyplot(umid6 ~ cam | manejo,
       data = rp[-63, ],
       ylab = expression("Umidade em 6 kPa" ~ (g ~ g^{-1})),
       xlab = "Profundidade (cm)") +
    as.layer(xyplot(fit ~ cam | manejo,
                    data = pred,
                    type = "l",
...
xyplot(dumid ~ cam | manejo,
       data = rp,
       ylab = expression(
           "Diferença de umidade em 0 e 6 kPa" ~ (g ~ g^{-1})),
       xlab = "Profundidade (cm)") +
    as.layer(xyplot(fit ~ cam | manejo,
                    data = pred,
                    type = "l",
